...
import { gaMuteVideo, gaPauseVideo, gaPlayVideo, gaShareVideo, gaUnmuteVideo } from "./gtagHelper";
...
export default function Home(
  ...
  const handleMutedClick = (indexVideo: number) => {
    if (isMuted) gaUnmuteVideo(url)
    else gaMuteVideo(url)
    setIsMuted(!isMuted);
  };

  const handleShare = (indexVideo: number) => {
    if (navigator.share) {
      navigator.share({
        title: 'La Braciera',
        text: 'La Braciera',
  ...
  const handleLikeVideo = (indexVideo: number) => {
    setVideoLiked((prev: any) => {
      const copy = [...prev];
      copy[indexVideo] = !copy[indexVideo];
      return copy;
    });
  }
